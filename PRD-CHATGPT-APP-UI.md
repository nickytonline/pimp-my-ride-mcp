...
server.registerResource(
  ...
  async () => {
    const template = await readFile("dist/widgets/templates/car-build-card.html", "utf-8");
    return {
      contents: [{
...
3. **UI result helper** (`src/lib/ui/response.ts`):
   ...
   export function createUIResult(data: unknown, textSummary: string, widget: WidgetDescriptor) {
     return {
